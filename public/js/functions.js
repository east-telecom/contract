...
function get_sum_text_ajax(sum, sum_text) {
    ...
    function afer_save_add_d_none_template() {
        ...
        $('.text_edit').attr('contenteditable', false).css('color', 'black');
    ...
    function table_create_tr(table, js_tarif_select) {
        ...
        let tr ='<tr class="js_tr_group js_'+js_tarif_select+'">\n' +
                    '<td colspan="6" class="text-bold text-left" contenteditable="true"><b>'+number+'</b>. &emsp; &emsp; '+tr_category+'</td>\n' +
                '</tr>\n'+

                '<tr class="js_tr_item">\n' +
                    '<td class="js_item_number"><b>'+number+'.1</b></td>\n' +
                    '<td class="text-left position-relative">' +
                        '<span contenteditable="true" class="text_edit td-span-text"><b>Абонентская плата**</b> <br/> Согласно тарифному плану</span>\n'+
                        ...
                        '</ul>\n'+
                    '</td>\n' +
                    '<td class="text_edit text-center" contenteditable="true">'+td3+'</td>\n' +
                    '<td class="text_edit text-center js_table_sena" contenteditable="true">0</td>\n' +
                    '<td class="text_edit text-center js_table_count" contenteditable="true">1</td>\n' +
    ...
    function horizontal_table_in_td_draw(this_tr) {
        ...
        $.each(body_object, function(index, one_object) {
            ...
            $.each(trs, function(i, one_tr) {
                ...
                if (!$(one_tr).hasClass('js_head_tr')) {
                    ...
                    if (!$(one_tr).hasClass('tr-kvartl') && !$(one_tr).hasClass('tr-footer-all-sum')) {
                        ...
                        body_td +=   '<td><span class="text_edit">' + s_ + '</span></td>\n' +
                                    '<td><span class="text_edit">' + one_object.td_count + '</span></td>\n' +
                                    '<td><span class="text_edit">' + t_ + '</span></td>';
                    }
                    else if($(one_tr).hasClass('tr-kvartl') && !$(one_tr).hasClass('tr-footer-all-sum')) {
                        body_td +=   '<td><span class="text_edit">' + number_format(sena_) + ' </span></td>\n' +
                                    '<td></td>\n' +
                                    '<td><span class="text_edit">' + number_format(total_) + '</span></td>';
        ...
        $.each(new_tr, function(n, one_tr) {
            ...
            if (!$(one_tr).hasClass('js_head_tr') && !$(one_tr).hasClass('tr-footer-all-sum')) {
                a = sub_total[n-1]
                a = number_format(a);
